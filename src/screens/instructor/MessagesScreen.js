// src/screens/instructor/MessagesScreen.js
  ...
  const fetchContacts = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        setContacts(data)
        if (studentId) {
          const match = data.find((c) => c.userId === studentId || c.id === studentId || c.studentId === studentId)
          if (match) setActiveContact(match)
        }
        if (!activeContact && studentName) {
          const match = data.find((c) => c.name && c.name.toLowerCase().includes(studentName.toLowerCase()))
          if (match) setActiveContact(match)
        }
      }
    } catch (e) {
      console.log('Error fetching chat contacts:', e)
    } finally {
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>Aucune conversation active pour vos offres</Text>
              </View>
            }
            renderItem={({ item }) => (
              <TouchableOpacity
